...
<!-- Copyright (C) 2005, 2006 Etienne Giraudy, InStranet Inc This library 
...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<!-- Parameter for specifying HTMLized sources location; if current dir, 
		use "./" If not passed, no links to sources are generated. because of back-compatibility 
		reasons. The source filename should be package.class.java.html The source 
		can have line no anchors like #11 -->
	<xsl:param name="htmlsrcpath"></xsl:param>

	<!-- xsl:key name="lbc-category-key" match="/BugCollection/BugInstance" 
		use="@category" / -->
		...
		<html>
			<head>
				...
				<script type="text/javascript">
					...
					function showbug(buguid,
					list) {
					var bugplaceholder =
					document.getElementById(buguid+'-ph-'+list);
					var bug =
					document.getElementById(buguid);
  ...
  /* ]]> */
				...
				<style type='text/css'>
					...
					font-weight: bold;
					background: #cccccc; margin: 0; padding: 0 5px 0
					5px;
					}
					.t-h {
					font-weight: normal;
				...
				<div id='content'>
					...
					<xsl:for-each select="/BugCollection/BugPattern">
						...
						<div>
							...
							<br />
							<xsl:value-of select="/BugCollection/BugPattern[@type=$b-t]/Details"
					...
					<xsl:for-each select="/BugCollection/BugInstance[not(@last)]">
						<div style="display:none;">
							<xsl:attribute name="id">b-uid-<xsl:value-of
								select="@instanceHash" />-<xsl:value-of select="@instanceOccurrenceNum" /></xsl:attribute>
										...
										<div class="b-r">
											<a>
												<xsl:attribute name="href"><xsl:value-of
													select="$htmlsrcpath" /><xsl:value-of select="../@sourcepath" />.html#<xsl:value-of
	...
	<xsl:template name="generateSummary">
		<div class='summary' id='bug-summary'>
			<h2>
				...
				<xsl:value-of select="/BugCollection/FindBugsSummary/@timestamp" />
			</h2>
			<table>
				<tr>
					<th>Package</th>
				...
				<xsl:for-each select="/BugCollection/FindBugsSummary/PackageStats">
					...
					<xsl:if test="@total_bugs!='0'">
						<tr>
							...
							<td class='summary-ratio'></td>
							<!-- <xsl:for-each select="ClassStats"> <xsl:if test="@bugs!='0'" 
								> <li> <xsl:value-of select="@class" /> - total: <xsl:value-of select="@bugs" 
		...
		<div class="bb">
			<a>
				...
				<xsl:attribute name="onclick">showbug('b-uid-<xsl:value-of
					select="@instanceHash" />-<xsl:value-of select="@instanceOccurrenceNum" />','<xsl:value-of
					select="$which-list" />');return false;</xsl:attribute>
				<span>
			...
			<div style="display:none;">
				<xsl:attribute name="id">b-uid-<xsl:value-of
					select="@instanceHash" />-<xsl:value-of select="@instanceOccurrenceNum" />-ph-<xsl:value-of
					select="$which-list" /></xsl:attribute>
				loading...
			</div>
		</div>
	</xsl:template>
			...
			<div class='ob-t'>
				<a>
					...
					<xsl:attribute name="onclick">toggle('category-<xsl:value-of
						select="$category" />');return false;</xsl:attribute>
					<xsl:value-of
					...
					:
					<span class='t-h'>
						<xsl:value-of select="$category-count-p1" />
						/
						<xsl:value-of select="$category-count-p2" />
						/
						<xsl:value-of select="$category-count-p3" />
						/
						<xsl:value-of select="$category-count-p4" />
					</span>
					)
				</a>
			</div>
		...
		<div class='ib-1'>
			<div class="ib-1-t">
				<a>
					<xsl:attribute name="href"></xsl:attribute>
					<xsl:attribute name="onclick">toggle('category-<xsl:value-of
						select="$category" />-and-code-<xsl:value-of select="$code" />');return false;</xsl:attribute>
					<xsl:value-of select="$code" />
					:
					<xsl:value-of select="/BugCollection/BugCode[@abbrev=$code]/Description" />
					(
					<xsl:value-of select="$code-count" />
					:
					<span class='t-h'>
						<xsl:value-of select="$code-count-p1" />
						/
						<xsl:value-of select="$code-count-p2" />
						/
						<xsl:value-of select="$code-count-p3" />
						/
						<xsl:value-of select="$code-count-p4" />
					</span>
					)
				</a>
			</div>
			<div style="display:none;">
				<xsl:attribute name="id">category-<xsl:value-of
					select="$category" />-and-code-<xsl:value-of select="$code" /></xsl:attribute>
		...
		<div class='ib-2'>
			<div class='ib-2-t'>
				<a>
					<xsl:attribute name="href"></xsl:attribute>
					<xsl:attribute name="onclick">toggle('category-<xsl:value-of
						select="$category" />-and-code-<xsl:value-of select="$code" />-and-bug-<xsl:value-of
						select="$bug" />');return false;</xsl:attribute>
					<xsl:attribute name="onmouseout">popUp(event,'tip-<xsl:value-of
						select="$bug" />');</xsl:attribute>
					<xsl:attribute name="onmouseover">popUp(event,'tip-<xsl:value-of
						select="$bug" />');</xsl:attribute>
					<xsl:value-of
						select="/BugCollection/BugPattern[@category=$category and @abbrev=$code and @type=$bug]/ShortDescription" />
					&#160;&#160;
					(
					<xsl:value-of select="$bug-count" />
					:
					<span class='t-h'>
						<xsl:value-of select="$bug-count-p1" />
						/
						<xsl:value-of select="$bug-count-p2" />
						/
						<xsl:value-of select="$bug-count-p3" />
						/
						<xsl:value-of select="$bug-count-p4" />
					</span>
					)
				</a>
			</div>
			<div style="display:none;">
				<xsl:attribute name="id">category-<xsl:value-of
					select="$category" />-and-code-<xsl:value-of select="$code" />-and-bug-<xsl:value-of
					select="$bug" /></xsl:attribute>
				<xsl:variable name="cat-code-type">
					category-
	...
	<xsl:template name="packages">
		...
		<xsl:variable name="package-count-p1">
			<xsl:if
				test="not(/BugCollection/FindBugsSummary/PackageStats[@package=$package]/@priority_1 != '')">
				0
			</xsl:if>
			<xsl:if
				test="/BugCollection/FindBugsSummary/PackageStats[@package=$package]/@priority_1 != ''">
				<xsl:value-of
		...
		<xsl:variable name="package-count-p2">
			<xsl:if
				test="not(/BugCollection/FindBugsSummary/PackageStats[@package=$package]/@priority_2 != '')">
				0
			</xsl:if>
			<xsl:if
				test="/BugCollection/FindBugsSummary/PackageStats[@package=$package]/@priority_2 != ''">
				<xsl:value-of
		...
		<xsl:variable name="package-count-p3">
			<xsl:if
				test="not(/BugCollection/FindBugsSummary/PackageStats[@package=$package]/@priority_3 != '')">
				0
			</xsl:if>
			<xsl:if
				test="/BugCollection/FindBugsSummary/PackageStats[@package=$package]/@priority_3 != ''">
				<xsl:value-of
		...
		<xsl:variable name="package-count-p4">
			<xsl:if
				test="not(/BugCollection/FindBugsSummary/PackageStats[@package=$package]/@priority_4 != '')">
				0
			</xsl:if>
			<xsl:if
				test="/BugCollection/FindBugsSummary/PackageStats[@package=$package]/@priority_4 != ''">
				<xsl:value-of
			...
			<div class='ob-t'>
				<a>
					...
					<xsl:attribute name="onclick">toggle('package-<xsl:value-of
						select="$package" />');return false;</xsl:attribute>
					<xsl:value-of select="$package" />
					(
					<xsl:value-of
					...
					:
					<span class='t-h'>
						<xsl:value-of select="$package-count-p1" />
						/
						<xsl:value-of select="$package-count-p2" />
						/
						<xsl:value-of select="$package-count-p3" />
						/
						<xsl:value-of select="$package-count-p4" />
					</span>
					)
				</a>
			</div>
	...
	<xsl:template name="classes">
		...
		<xsl:variable name="class-count-p1">
			<xsl:if
				test="not(/BugCollection/FindBugsSummary/PackageStats[@package=$package]/ClassStats[@class=$class and @bugs != '0']/@priority_1 != '')">
				0
			</xsl:if>
			<xsl:if
				test="/BugCollection/FindBugsSummary/PackageStats[@package=$package]/ClassStats[@class=$class and @bugs != '0']/@priority_1 != ''">
				<xsl:value-of
		...
		<xsl:variable name="class-count-p2">
			<xsl:if
				test="not(/BugCollection/FindBugsSummary/PackageStats[@package=$package]/ClassStats[@class=$class and @bugs != '0']/@priority_2 != '')">
				0
			</xsl:if>
			<xsl:if
				test="/BugCollection/FindBugsSummary/PackageStats[@package=$package]/ClassStats[@class=$class and @bugs != '0']/@priority_2 != ''">
				<xsl:value-of
		...
		<xsl:variable name="class-count-p3">
			<xsl:if
				test="not(/BugCollection/FindBugsSummary/PackageStats[@package=$package]/ClassStats[@class=$class and @bugs != '0']/@priority_3 != '')">
				0
			</xsl:if>
			<xsl:if
				test="/BugCollection/FindBugsSummary/PackageStats[@package=$package]/ClassStats[@class=$class and @bugs != '0']/@priority_3 != ''">
				<xsl:value-of
		...
		<xsl:variable name="class-count-p4">
			<xsl:if
				test="not(/BugCollection/FindBugsSummary/PackageStats[@package=$package]/ClassStats[@class=$class and @bugs != '0']/@priority_4 != '')">
				0
			</xsl:if>
			<xsl:if
				test="/BugCollection/FindBugsSummary/PackageStats[@package=$package]/ClassStats[@class=$class and @bugs != '0']/@priority_4 != ''">
				<xsl:value-of
		...
		<div class='ib-1'>
			<div class="ib-1-t">
				<a>
					<xsl:attribute name="href"></xsl:attribute>
					<xsl:attribute name="onclick">toggle('package-<xsl:value-of
						select="$package" />-and-class-<xsl:value-of select="$class" />');return false;</xsl:attribute>
					<xsl:value-of select="$class" />
					(
					<xsl:value-of select="$class-count" />
					:
					<span class='t-h'>
						<xsl:value-of select="$class-count-p1" />
						/
						<xsl:value-of select="$class-count-p2" />
						/
						<xsl:value-of select="$class-count-p3" />
						/
						<xsl:value-of select="$class-count-p4" />
					</span>
					)
				</a>
			</div>
			<div style="display:none;">
				<xsl:attribute name="id">package-<xsl:value-of
					select="$package" />-and-class-<xsl:value-of select="$class" /></xsl:attribute>
		...
		<div class='ib-2'>
			<div class='ib-2-t'>
				<a>
					<xsl:attribute name="href"></xsl:attribute>
					<xsl:attribute name="onclick">toggle('package-<xsl:value-of
						select="$package" />-and-class-<xsl:value-of select="$class" />-and-type-<xsl:value-of
						select="$type" />');return false;</xsl:attribute>
					<xsl:attribute name="onmouseout">popUp(event,'tip-<xsl:value-of
						select="$type" />')</xsl:attribute>
					<xsl:attribute name="onmouseover">popUp(event,'tip-<xsl:value-of
						select="$type" />')</xsl:attribute>
					<xsl:value-of
						select="/BugCollection/BugPattern[@type=$type]/ShortDescription" />
					&#160;&#160;
					(
					<xsl:value-of select="$bug-count" />
					)
			...
			<div style="display:none;">
				<xsl:attribute name="id">package-<xsl:value-of
					select="$package" />-and-class-<xsl:value-of select="$class" />-and-type-<xsl:value-of
					select="$type" /></xsl:attribute>
				<xsl:variable name="package-class-type">
					package-
					<xsl:value-of select="$package" />
					-and-class-
					<xsl:value-of select="$class" />
